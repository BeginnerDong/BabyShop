		...
		<view class="myaddress-lis" v-for="(item,index) in mainData" :key="index" >
			...
			<view class="seltBox">
				<view class="L" :data-id="item.id" @click="updateAddress($event.currentTarget.dataset.id)">
					<image class="icon" :src="item.isdefault==1?'../../static/images/addressl-icon.png':'../../static/images/addressl-icon1.png'"
					 mode=""></image>
					默认地址
				</view>
				<view class="R fs12 color9 flexEnd">
					<view class="child flexEnd mgr20" :data-id="item.id" @click="Router.navigateTo({route:{path:'/pages/user_addressAdd/user_addressAdd?id='+$event.currentTarget.dataset.id}})">
						<image src="../../static/images/addressl-icon2.png" mode=""></image>编辑
					</view>
					<view class="child flexEnd" :data-id="item.id" @click="deleteAddress($event.currentTarget.dataset.id)">
						<image src="../../static/images/addressl-icon3.png" mode=""></image>删除
					</view>
				</view>
			</view>
		</view>

		<view class="addAdrsBtn pdlr4 flexCenter fs15" @click="Router.navigateTo({route:{path:'/pages/user_addressAdd/user_addressAdd'}})">
			<image class="mgr10" style="width: 30rpx;height: 30rpx;" src="../../static/images/addressl-icon4.png" mode=""></image>新增
		</view>
